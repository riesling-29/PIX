"""Verified OCEL 2.0 interchange without changing canonical information.

Profiles describe the emitted representation, not official schema certification.
...
from pix.ocel.ingest import ImportResult, import_ocel
from pix.ocel.model import OCEL, OCEL_EPOCH, ValueType
...
class ExportIssue:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.code, str) or not self.code.strip():
            raise ValueError("ExportIssue.code must be a nonempty string")
        if not isinstance(self.message, str) or not self.message.strip():
            raise ValueError("ExportIssue.message must be a nonempty string")
        if not isinstance(self.at, tuple) or not all(
            isinstance(p, str) for p in self.at
        ):
            raise TypeError("ExportIssue.at must be a tuple of strings")
...
class ExportError(ValueError):
    ...
    def __init__(
        ...
        *,
        ...
        self.cleanup_issues: tuple[ExportIssue, ...] = ()
...
class ExportResult:
    ...
    ``output_sha256`` covers the exact published bytes, including compression.
    ``cleanup_issues`` identifies a staging hardlink that could not be removed;
    it does not change the verified, committed destination's successful status.
    """
    ...
    cleanup_issues: tuple[ExportIssue, ...] = ()
    status: Literal["published"] = field(default="published", init=False)
    ...
    def __post_init__(self) -> None:
        ...
        if self.format not in _PROFILES or self.profile != _PROFILES[self.format]:
            raise ValueError(
                "format and profile must describe a supported representation"
            )
        if self.encoding not in {"identity", "gzip"}:
            raise ValueError("encoding must be identity or gzip")
        if self.format == "ocel20-sqlite" and self.encoding != "identity":
            raise ValueError("SQLite gzip encoding is not supported")
        if not isinstance(self.cleanup_issues, tuple) or not all(
            isinstance(issue, ExportIssue) for issue in self.cleanup_issues
        ):
            raise TypeError("cleanup_issues must be a tuple of ExportIssue")
        if not isinstance(
            self.source_canonical_digest, CanonicalDigest
        ) or not isinstance(self.roundtrip_canonical_digest, CanonicalDigest):
            raise TypeError("source and roundtrip digests must be CanonicalDigest")
...
_FORMATS = {
    "json": "ocel20-json",
    "ocel20-json": "ocel20-json",
    "xml": "ocel20-xml",
    "ocel20-xml": "ocel20-xml",
    "sqlite": "ocel20-sqlite",
    "ocel20-sqlite": "ocel20-sqlite",
}
_PROFILES = {
    "ocel20-json": "pix.ocel20-json.native-primitives.v1",
    "ocel20-xml": "pix.ocel20-xml.no-namespace.relobj.v1",
...
def export_ocel(
    ...
    *,
    ...
    XML emits the named ``relobj`` profile without an XML namespace. JSON/XML
    gzip is selected by a ``.gz`` destination suffix and has deterministic headers.
    ...
    if selected is None:
        raise ExportError(
            "unsupported_format",
            "Supported formats are OCEL 2.0 JSON, XML, and SQLite.",
            path=path_text,
        )
    compressed = destination.suffix.lower() == ".gz"
    if compressed and selected == "ocel20-sqlite":
        raise ExportError(
            "unsupported_encoding",
            "Gzip is supported only for JSON and XML.",
            path=path_text,
        )
    ...
    if not built.valid:
        raise ExportError(
            "semantic_invalid",
            "Source OCEL failed PIX semantic validation.",
            path=path_text,
            semantic_report=built.report,
            issues=tuple(
                ExportIssue(i.code, i.message, i.at) for i in built.report.issues
            ),
        )
    ...
    if selected in {"ocel20-sqlite", "ocel20-xml"}:
        issues = (
            _sqlite_preflight(normalized)
            if selected == "ocel20-sqlite"
            else _xml_preflight(normalized)
        )
        if issues:
            raise ExportError(
                "unrepresentable",
                f"{selected} cannot preserve the supplied OCEL exactly.",
                path=path_text,
    ...
    if not overwrite and os.path.lexists(destination):
        raise ExportError(
            "destination_exists",
            "Destination exists; overwrite was not requested.",
            path=path_text,
            source_canonical_digest=digest,
        )

    stage: Path | None = None
    failure: ExportError | None = None
    result: ExportResult | None = None
    published = False
    phase = "serialize"
    try:
        suffix = {
            "ocel20-json": ".jsonocel",
            "ocel20-xml": ".xmlocel",
            "ocel20-sqlite": ".sqlite",
        }[selected]
        if compressed:
            suffix += ".gz"
        ...
        elif selected == "ocel20-xml":
            _write_xml(normalized, stage)
        else:
        ...
        if not imported.valid or imported.canonical_digest != digest:
            raise ExportError(
                "roundtrip_failed",
                "Serialized artifact did not preserve canonical identity.",
                path=path_text,
                source_canonical_digest=digest,
        ...
        result = ExportResult(
            ...
            roundtrip_canonical_digest=imported.canonical_digest,
            encoding="gzip" if compressed else "identity",
        )
        phase = "publish"
        stage = _publish(stage, destination, overwrite=overwrite)
        published = True
        ...
        if stage is not None:
            try:
                ...
                issue = ExportIssue("temporary_cleanup_failed", str(exc), (str(stage),))
                if failure is not None:
                    failure.cleanup_issues = (issue,)
                elif published and result is not None:
                    result = replace(result, cleanup_issues=(issue,))
                # An unexpected exception remains the primary failure; cleanup
                # must not mask it. A committed artifact remains a success.
    assert result is not None
    return result


def _publish(stage: Path, destination: Path, *, overwrite: bool) -> Path | None:
    if overwrite:
        os.replace(stage, destination)
        return None
    if os.name == "nt":
        os.rename(stage, destination)  # Windows rename refuses an existing destination.
        return None
...
def _write_json(log: OCEL, path: Path) -> None:
    ...
    for rel in log.e2o:
        e2o.setdefault(rel.event, []).append(
            {"objectId": rel.object, "qualifier": rel.qualifier}
        )
    for rel in log.o2o:
        o2o.setdefault(rel.source, []).append(
            {"objectId": rel.target, "qualifier": rel.qualifier}
        )
    document = {
        "eventTypes": [
            {
                "name": typ.name,
                "attributes": [
                    {"name": a.name, "type": a.type.value} for a in typ.attributes
                ],
            }
        ...
        "events": [
            {
                ...
                "time": _time(e.time),
                "attributes": [
                    {"name": a.name, "value": _value(a.value)} for a in e.attributes
                ],
        ...
        "objects": [
            {
                ...
                "type": obj.type,
                "attributes": [
                    {"name": a.name, "value": _value(a.value), "time": _time(a.time)}
...
def _write_bytes(path: Path, payload: bytes) -> None:
    with path.open("wb") as stream:
        if path.suffix.lower() == ".gz":
            # No source filename and no wall-clock timestamp enter the artifact.
            with gzip.GzipFile(
...
def _xml_preflight(log: OCEL) -> tuple[ExportIssue, ...]:
    issues: list[ExportIssue] = []

    def check(value: str, at: tuple[str, ...]) -> None:
        if any(
            not (
                character in "\t\n\r"
    ...
    for kind, declarations in (
        ("eventTypes", log.event_types),
        ("objectTypes", log.object_types),
    ):
        for typ in declarations:
            check(typ.name, (kind, typ.name, "name"))
            for attr in typ.attributes:
                check(attr.name, (kind, typ.name, "attributes", attr.name))
    for kind, entities in (("events", log.events), ("objects", log.objects)):
        for entity in entities:
            check(entity.id, (kind, entity.id, "id"))
            check(entity.type, (kind, entity.id, "type"))
            for attr in entity.attributes:
                check(attr.name, (kind, entity.id, "attributes", attr.name, "name"))
                if isinstance(attr.value, str):
                    check(
                        attr.value, (kind, entity.id, "attributes", attr.name, "value")
                    )
    for index, relation in enumerate(log.e2o):
        for field_name in ("event", "object", "qualifier"):
            check(getattr(relation, field_name), ("e2o", str(index), field_name))
    for index, relation in enumerate(log.o2o):
        for field_name in ("source", "target", "qualifier"):
            check(getattr(relation, field_name), ("o2o", str(index), field_name))
    return tuple(issues)
...
def _write_xml(log: OCEL, path: Path) -> None:
    root = ET.Element("log")
    for kind, declarations in (
        ("object", log.object_types),
        ("event", log.event_types),
    ):
        container = ET.SubElement(root, f"{kind}-types")
        for typ in declarations:
            element = ET.SubElement(container, f"{kind}-type", {"name": typ.name})
            attributes = ET.SubElement(element, "attributes")
            for attr in typ.attributes:
                ET.SubElement(
                    attributes,
                    "attribute",
                    {"name": attr.name, "type": attr.type.value},
                )
    e2o: dict[str, list[tuple[str, str]]] = {}
    o2o: dict[str, list[tuple[str, str]]] = {}
    for relation in log.e2o:
        e2o.setdefault(relation.event, []).append((relation.object, relation.qualifier))
    for relation in log.o2o:
        o2o.setdefault(relation.source, []).append(
            (relation.target, relation.qualifier)
        )
    for kind, entities, relations in (
        ("object", log.objects, o2o),
        ("event", log.events, e2o),
    ):
        container = ET.SubElement(root, f"{kind}s")
        for entity in entities:
            fields = {"id": entity.id, "type": entity.type}
            if kind == "event":
                fields["time"] = _time(entity.time)
            element = ET.SubElement(container, kind, fields)
            attributes = ET.SubElement(element, "attributes")
            for attr in entity.attributes:
                fields = {"name": attr.name}
                if kind == "object":
                    fields["time"] = _time(attr.time)
                ET.SubElement(attributes, "attribute", fields).text = _xml_value(
                    attr.value
                )
            objects = ET.SubElement(element, "objects")
            for object_id, qualifier in relations.get(entity.id, ()):
                ET.SubElement(
                    objects, "relobj", {"object-id": object_id, "qualifier": qualifier}
                )
    # ElementTree escapes attribute whitespace. XML normalizes literal CR in
    # element text, so a numeric reference is also required there.
    serialized = ET.tostring(root, encoding="unicode").replace("\r", "&#13;")
    _write_bytes(
        path,
        ...
        for typ in declarations:
            ...
            for attr in typ.attributes:
                ...
                if "\x00" in attr.name or folded in seen:
                    issues.append(
                        ExportIssue(
                            "sqlite_column_collision",
                            "Attribute is reserved, duplicated ignoring ASCII case, or contains NUL.",
        ...
        for entity in entities:
            for attr in entity.attributes:
                ...
                if (
                    ...
                    issues.append(
                        ExportIssue(
                            "sqlite_negative_zero",
                            "REAL storage does not preserve negative zero.",
                            (kind, entity.id, attr.name),
                        )
                    )
    return tuple(issues)
...
def _write_sqlite(log: OCEL, path: Path) -> None:
    ...
    try:
        with connection:
            ...
            for kind, declarations in (
                ...
                for index, typ in enumerate(declarations):
                    ...
                    _insert(
                        connection,
                        f"{kind}_map_type",
                        {"ocel_type": typ.name, "ocel_type_map": physical},
                    )
                    fixed = ["ocel_id TEXT NOT NULL", "ocel_time TIMESTAMP NOT NULL"]
                    fixed.append(
                        "ocel_changed_field TEXT"
                        if kind == "object"
                        else "PRIMARY KEY (ocel_id)"
                    )
                    columns = [
                        f"{_quote(a.name)} {_SQL_TYPES[a.type]}" for a in typ.attributes
                    ]
                    # Table constraints must follow all column declarations.
                    definitions = fixed[:2] + columns + fixed[2:]
                    connection.execute(
                        f"CREATE TABLE {_quote(mappings[kind, typ.name])} ({', '.join(definitions)})"
                    )
            for event in log.events:
                _insert(
                    connection, "event", {"ocel_id": event.id, "ocel_type": event.type}
                )
            ...
            for obj in log.objects:
                _insert(
                    connection, "object", {"ocel_id": obj.id, "ocel_type": obj.type}
                )
                table = mappings["object", obj.type]
                baseline = {
                    "ocel_id": obj.id,
                    "ocel_time": _time(OCEL_EPOCH),
                ...
                for attr in obj.attributes:
                    if attr.time != OCEL_EPOCH:
                        _insert(
                            ...
                            {
                                "ocel_id": obj.id,
                                "ocel_time": _time(attr.time),
                                "ocel_changed_field": attr.name,
            ...
            for rel in log.e2o:
                _insert(
                    ...
                    "event_object",
                    {
                        "ocel_event_id": rel.event,
                        "ocel_object_id": rel.object,
                        "ocel_qualifier": rel.qualifier,
                    },
                )
            for rel in log.o2o:
                _insert(
                    ...
                    "object_object",
                    {
                        "ocel_source_id": rel.source,
                        "ocel_target_id": rel.target,
                        "ocel_qualifier": rel.qualifier,
                    },
                )
    finally:
